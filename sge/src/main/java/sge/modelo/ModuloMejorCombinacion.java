...
public class ModuloMejorCombinacion {
	...
	public Recomendacion calcularMejorCombinacion(List<DispositivoVO> dispositivos) {
			
		Recomendacion reco = new Recomendacion();
		
		DispositivoVO[] nodos = new DispositivoVO[dispositivos.size()];	
	...
	private void agregarRestriccionMaxMin(SimplexFacade simplexFacade, int i, int length, DispositivoVO nodo) {
	...
	private double[] coeficientesFunEco(DispositivoVO[] arrayDispo) {
	...
	private double[] coeficientesConsumokWh(DispositivoVO[] arrayDispo) {
		double[] unArray = new double[arrayDispo.length];
		for (int i = 0; i < arrayDispo.length; i++) {
